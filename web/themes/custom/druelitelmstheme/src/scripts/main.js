...
(function ($, Drupal, drupalSettings) {
  Drupal.druelitelmstheme = {
    settings: drupalSettings.druelitelmstheme || {
      ...
      if ($('body.anonymous-slider', context).length) {
        ...
        setInterval(() => {
          ...
          $(`.anonymous-slider .slider-item[data-id="${currentSlide}"]`)
            .removeClass('hide')
            .fadeIn();
        }, Drupal.druelitelmstheme.settings.homepageSlideSpeed);
      }
    },
    ...
    stepsVisibility(context) {
      if (!$('#block-druelitelmstheme-lp-steps-block', context).length) {
        return;
      }
  ...
  Drupal.behaviors.druelitelmstheme = {
    attach(context) {
      Drupal.druelitelmstheme.frontpageSlider(context);
      Drupal.druelitelmstheme.anonymousUserForms(context);
      Drupal.druelitelmstheme.searchBar(context);
      Drupal.druelitelmstheme.trainingCatalog(context);
      Drupal.druelitelmstheme.privateMessageRecipients(context);
      Drupal.druelitelmstheme.stepsVisibility(context);
      Drupal.druelitelmstheme.mobileMenu(context);
      Drupal.druelitelmstheme.fileWidget(context);
      Drupal.druelitelmstheme.packageFileInput(context);
      Drupal.druelitelmstheme.viewOpignoActivitiesBank(context);

      $('iframe').iFrameResize();

      $('a[href="#documents-library"]', context).once().click(() => {
        Drupal.druelitelmstheme.formatTFTOperations(context);
      });
